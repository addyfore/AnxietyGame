...
{ 
    ...
    public override void castSpell()
    {
        ...
        if (hit.collider)
        {
            ...
            {
                monsterStone.GetComponentInChildren<Light2D>().enabled = true;
...

        {
            ...
            for (int i = 0; i < hero.GetComponent<Player>().numberOfPools -1; i++)
            {
                Debug.Log("for statement for the bright spell cycling");

                if (player.instantiatedPools[i].activeSelf == false)
                {
                    Debug.Log("found inactive pool.");
                    activeLightPool = player.instantiatedPools[i];
            ...
            var instantiatedPoolsTemp = new List<GameObject>(player.instantiatedPools.Capacity);

            if (forLoopFoundDisabledPoolToImplement == false)
            {

                Debug.Log("No pools are disabled. Recycling pool.");
                activeLightPool = player.instantiatedPools[0];

                player.instantiatedPools.Remove(activeLightPool);
                instantiatedPoolsTemp.AddRange(player.instantiatedPools);
            ...
            else
            {
                ...
                for (int i = 0; i < hero.GetComponent<Player>().numberOfPools - 1; i++)
                {
                    ...
                    if (player.instantiatedPools[i] != activeLightPool)
                    {

                        instantiatedPoolsTemp.Insert(i, player.instantiatedPools[i]);
                    }
                }
                instantiatedPoolsTemp.Insert(3, activeLightPool);
            ...
            player.instantiatedPools.Clear();
            player.instantiatedPools.AddRange(instantiatedPoolsTemp);
